...
/// Memeriksa dan meminta izin lokasi (saat aplikasi digunakan).
///
/// Mengembalikan `true` jika izin diberikan.
/// Melemparkan error jika izin ditolak, dibatasi, atau terjadi kesalahan.
Future<bool> checkLocationPermission() async {
  ...
  try {
    if (!Platform.isAndroid && !Platform.isIOS) {
      debugPrint("Pengecekan izin $permissionName tidak relevan untuk platform ini.");
      return true; 
    }

    LocationPermission permission = await Geolocator.checkPermission();
    debugPrint("Status awal izin $permissionName: $permission");
    ...
    debugPrint(
      "Status baru izin $permissionName setelah permintaan: $permission",
    );
    ...
    if (permission == LocationPermission.always || permission == LocationPermission.whileInUse) {
      ...
      debugPrint("Izin $permissionName tidak diberikan setelah permintaan (status: $permission).");
      return Future.error("Izin $permissionName tidak diberikan setelah permintaan.");
    }
  } catch (e) {
    debugPrint("Error saat memeriksa atau meminta izin $permissionName: $e");
